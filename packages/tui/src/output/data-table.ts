...
const isDataObjectRow = <T>(row: DataTableRow<T>): row is DataTableObjectRow<T> => {
  return !Array.isArray(row) && 'cells' in row && typeof row.cells === 'object' && row.cells !== null;
};
  ...
  return promptUntilValid(options, async () => {
    ...
    const visibleRows = (): Array<VisibleRow<T>> => {
      return options.rows
        ...
        .filter(({ row }) => options.filter?.(query, row) ?? rowLabel(headers, row).toLowerCase().includes(query.toLowerCase()));
    };
