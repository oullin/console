...
Install the package from the workspace or package registry that contains `@ollin/tui`.
...
Inside this repository, `@ollin/tui` is a workspace package. The docs package depends on it with `workspace:*`, so examples compile against the package source.
