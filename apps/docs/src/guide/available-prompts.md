...
`@ollin/console` provides prompt helpers for text input, choice selection, search flows, confirmation, pausing, and autocomplete.
...
```ts
import { autocomplete, confirm, multiselect, multisearch, number, password, pause, search, select, suggest, text, textarea } from '@ollin/console';
...
```ts
import { confirm, select, text } from '@ollin/console';
