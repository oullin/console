...
`@ollin/console` represents terminal IO through `PromptEnvironment`. The default environment reads from the process input and writes to process output. Tests and non-standard runtimes can replace those streams.

```ts
import { promptEnvironment, terminalSize } from '@ollin/console';
...
```ts
import { createMemoryOutput, createScriptedInput, promptEnvironment, terminalSize, text, withPromptEnvironment } from '@ollin/console';
...
```ts
import { createMemoryOutput, createScriptedInput, text, withPromptEnvironment } from '@ollin/console';
